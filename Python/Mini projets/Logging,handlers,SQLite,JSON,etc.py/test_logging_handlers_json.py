...
# JSON
json_path = os.path.join(folder, "users.json")

def load_data():
    if os.path.exists(json_path):
        with open(json_path, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"Thomas": "0894", "Megane": "2510", "Jean" : "AbCd", "Camille" : "cacahuete"}
...
# VERIFICATION MOT DE PASSE 
    ...
    while True :
        ...
        if password == users[user]:
            ...
            if attempts >= 3:
                logging.warning(f"{user}'s account is temporarily suspented after 3 attempts.")
